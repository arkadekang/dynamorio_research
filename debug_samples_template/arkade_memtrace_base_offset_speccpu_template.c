...
static void wrap_malloc_pre(void *wrapcxt, OUT void **user_data) {
    ...
    if (is_user) {
        dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "[ARKADE MALLOC CALLED] Malloc (User Code) (size: %zu)\n", size);
    } else {
        dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "[ARKADE MALLOC CALLED] Malloc (Nonuser Code) (size: %zu)\n", size);
    }
    ...
    if (mod != NULL) {
        ...
        //if (sym_res == DRSYM_SUCCESS | sym_res == DRSYM_ERROR_LINE_NOT_AVAILABLE) {
            ...
            dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "[ARKADE MALLOC CALLED] Caller: Unknown, sym_res: %d\n", sym_res);
        }
    } else {
        dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "[ARKADE MALLOC CALLED] Caller: Unknown, mode: NULL\n");
    }
    ...
    if (double_free == 0) {
        if (mod != NULL) {
            ...
            if (sym_res == DRSYM_SUCCESS | sym_res == DRSYM_ERROR_LINE_NOT_AVAILABLE) {
                ...
                dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "Caller: Unknown, sym_res: %d\n", sym_res);
            }
        } else {
            dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "Caller: Unknown, mode: NULL\n");
        }
    } else {
        dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "[ARKADE FREE] Double Free detected\n");
    }
...
reg_t get_register_value(dr_mcontext_t *mc, reg_id_t reg_id) {
    ...
    dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "[WARNING] Unhandled register: %s (reg_id: %d)\n", get_register_name(reg_id), reg_id);
...
static void mem_access_callback(void *drcontext, app_pc instr_addr, app_pc abs_addr, app_pc rel_addr, int base_reg, int index_reg, int64_t scale, int64_t offset, int is_write) {
    ...
    if (base_reg != DR_REG_NULL) {
        ...
        if (!dr_get_mcontext(drcontext, &mc)) {
            dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "[ERROR] Failed to get register context at runtime!\n");
            return;
        }
    ...
    if (addr != NULL && is_in_active_region(addr, &region, 1)) {
        ...
        if ( base_reg_val == (reg_t)(region->base_address) ) {/*{{{*/
            ...
            dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "\n\t\t\t[RESULT] -> ");
            dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "[FAIL] %s at Actual Addr: %p, Base Addr: %p, Malloc Ret Addr: %p\n",
        ...
        if (instr_addr != NULL ) {
            ...
            if (mod != NULL) {
                if (debug_src_print == true) {
                    ...
                    //if (sym_res == DRSYM_SUCCESS || sym_res == DRSYM_ERROR_LINE_NOT_AVAILABLE) {
                    if (sym_res == DRSYM_SUCCESS) {
                        dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "\t\t\t\t\t\tCaller: %s (%s:%d)\n\n", sym.name, sym.file, sym.line);
                    } else {
                        dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "\t\t\t\t\t\tCaller: Unknown, sym_res: %d\n\n", sym_res);
                    }
                ...
                dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "\t\t\t\t\t\tModule lookup failed for instruction address: %p\n\n", instr_addr);
            }
        } else {
            dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "\t\t\t\t\t\tInvalid instruction address: %p\n\n", instr_addr);
        }
    }
}
...
static bool should_ignore_memory_access(reg_id_t base_reg) {
    ...
    //    dr_log(NULL, DR_LOG_MASK_BASEOFFSET, "\t\t\tbase_reg is DR_REG_RSP\n");
    //    return true;
    //}
    //if (base_reg == DR_REG_RBP) {
    //    dr_log(NULL, DR_LOG_MASK_BASEOFFSET, "\t\t\tbase_reg is DR_REG_RBP\n");
    //    return true;
    //}
        ...
        if ( instr_reads_memory(instr) ) {
            for (int i = 0; i < instr_num_srcs(instr); i++) {
                ...
                if (opnd_is_base_disp(opnd)) {
                    ...
                    dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "[WARNING] Unsupported operand type. Skipping.\n");
                    continue;
                }
...
DR_EXPORT void dr_client_main(client_id_t id, int argc, const char *argv[]) {
    ...
    dr_log(NULL, DR_LOG_MASK_BASEOFFSET, 2, "Client 'memtrace' initializing\n");
}
