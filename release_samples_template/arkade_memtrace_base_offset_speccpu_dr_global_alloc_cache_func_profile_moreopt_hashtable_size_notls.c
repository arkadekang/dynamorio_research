...
// uthash structure
typedef struct {
    void *key;         // malloc address
    int value;         // region index
...
static void wrap_malloc_pre(void *wrapcxt, OUT void **user_data) {
    ...
    #ifdef USE_TLS_FOR_SIZE_DELIVERY
        void *drcontext = drwrap_get_drcontext(wrapcxt);
        per_thread_t *data = (per_thread_t *)drmgr_get_tls_field(drcontext, tls_idx);
        data->size = size; // Pass 'size' to wrap_malloc_post
    #else
        *user_data = (void *)(uintptr_t)size; // Pass 'size' to wrap_malloc_post through user_data
    #endif
...
void init_region_table() {
    ...
    regions = dr_global_alloc(sizeof(memory_region_t) * region_capacity);
...
void hash_table_insert(void *key, int value) {
    ...
    entry->key = key;
    entry->value = value;
    HASH_ADD_PTR(hash_table, key, entry);
}

// Lookup a key in the hash table
static int hash_table_lookup(void *key) {
    HashEntry *entry = NULL;
    HASH_FIND_PTR(hash_table, &key, entry);
    if (entry != NULL) {
        return entry->value;
    }
    return -1; // not found
...
static int lookup_region_index(void *ptr) {
    ...
    if (index >= 0 && index < region_capacity) {
        ...
        if (region->is_active && region->base_address == ptr) {
            // Valid fast path hit
            return index;
        }
    }
    ...
    for (int i = 0; i < region_capacity; i++) {
        if (regions[i].is_active && regions[i].base_address == ptr) {
            return i;
        }
    }
...
static void wrap_malloc_post(void *wrapcxt, void *user_data) {
    ...
    #else
        size_t size = (size_t)(uintptr_t)user_data; // Obtain size through user_data passed from wrap_malloc_pre
    #endif
...
static void wrap_free_pre(void *wrapcxt, OUT void **user_data) {
    ...
    int index = lookup_region_index(ptr);
